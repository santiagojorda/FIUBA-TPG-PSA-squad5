...
class Database: 
    def __init__(self, db_url): 
        ...
        Base_ticket.metadata.drop_all(bind=self.engine)
        Base_product.metadata.drop_all(bind=self.engine)
        Base_version.metadata.drop_all(bind=self.engine)
        Base_consulta.metadata.drop_all(bind=self.engine)
        Base_incidente.metadata.drop_all(bind=self.engine)
        Base_incidente_por_tarea.metadata.drop_all(bind=self.engine)
        Base_severity.metadata.drop_all(bind=self.engine)
        ...
        Base_severity.metadata.create_all(bind=self.engine)
